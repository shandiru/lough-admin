...
const LeaveApplyModal = ({ onClose, onSuccess }) => {
  ...
  // When toggling to hourly mode, lock endDate = startDate
  const toggleHourly = () => {
    setConflicts(null);
    setForm(f => ({
      ...f,
  ...
  const handleStartDate = (v) => {
    ...
    setForm(f => ({
      ...f,
      startDate: v,
      // In hourly mode keep endDate in sync; in full-day mode guard against endDate < startDate
      endDate: f.isHourly ? v : (f.endDate && f.endDate >= v ? f.endDate : v),
    }));
  };

  // Human-readable duration for hourly mode
      ...
      <div className="bg-white rounded-[28px] shadow-2xl w-full max-w-md p-8 relative max-h-[92vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {conflictsByDay && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
              ...
              <p className="text-xs text-amber-600 mb-3">
                Cancel or reschedule these bookings before applying for leave.
              </p>
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-[var(--color-brand)] text-white font-bold py-3.5 rounded-2xl flex items-center justify-center gap-2 hover:opacity-90 transition disabled:opacity-60">
