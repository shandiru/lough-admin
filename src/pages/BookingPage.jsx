...
function CompleteBookingModal({ booking, onClose, onDone }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success('Booking marked as completed' + (amount > 0 ? ` · £${amount.toFixed(2)} balance recorded` : ''));
      onDone();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div>
            <h3 className="font-bold text-gray-800 flex items-center gap-2"><CheckCircle2 size={18} className="text-green-500"/> Complete Booking</h3>
            <p className="text-xs text-gray-400 mt-0.5">{booking.bookingNumber} · {booking.customerName}</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-full hover:bg-gray-100"><X size={18}/></button>
        </div>
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="bg-gray-50 rounded-xl p-3"><p className="text-xs text-gray-400 mb-0.5">Service</p><p className="font-semibold text-gray-800">{booking.service?.name}</p></div>
            <div className="bg-gray-50 rounded-xl p-3"><p className="text-xs text-gray-400 mb-0.5">Total</p><p className="font-bold text-gray-800">£{(booking.totalAmount / 100).toFixed(2)}</p></div>
            <div className="bg-gray-50 rounded-xl p-3"><p className="text-xs text-gray-400 mb-0.5">Already Paid</p><p className="font-bold text-[#22B8C8]">£{(booking.paidAmount / 100).toFixed(2)}</p></div>
          ...
          {hasBalance ? (
            <div className="space-y-3">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={inputCls + ' pl-7 ' + (noBalance ? 'opacity-40' : '')}
                  />
                </div>
              </div>
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                />
                <span className="text-sm text-gray-600">No balance collected (waived / cash handled separately)</span>
              </label>
            </div>
          ) : (
...
function BookingDrawer({ booking, onClose, onUpdated }) {
  ...
  return (
    <>
        ...
        <div className="p-5 space-y-5">
          ...
          {booking.status!=='cancelled' && (
            <div>
              ...
              {status==='completed' && <p className="text-xs text-green-500 mt-1.5 flex items-center gap-1"><CheckCircle2 size={11}/> Will prompt to record any balance payment</p>}
            </div>
          )}
        </div>
      </div>
      ...
      {showComplete && <CompleteBookingModal booking={booking} onClose={()=>{setShowComplete(false);setStatus(booking.status);}} onDone={()=>{setShowComplete(false);onClose();onUpdated();}}/>}
    </>
  );
}
