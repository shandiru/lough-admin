...
import { verifySetupPassword, checkTokenStatus } from "../api/auth";
      ...
      <div className="w-full max-w-md bg-white/90 backdrop-blur-md shadow-2xl rounded-2xl p-8">
        ...
        <div className="mb-6">
          ...
          <input
            ...
            value={email || ""}
            disabled
            className="w-full rounded-lg px-4 py-2 bg-gray-100 text-gray-500 cursor-not-allowed"
          />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full rounded-lg px-4 py-2 bg-brand-light/30 placeholder-gray-500 focus:bg-white focus:ring-2 focus:ring-brand outline-none transition disabled:opacity-50"
              />
              <button
                type="button"
                onClick={() => setShowPass(!showPass)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-brand text-sm"
              >
                {showPass ? "Hide" : "Show"}
              </button>
            </div>
          ...
          <button
            ...
            className="w-full bg-brand text-white py-2.5 rounded-lg font-medium shadow-md hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {status === "loading"
              ? "Processing..."
              : status === "success"
